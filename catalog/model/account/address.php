...
class ModelAccountAddress extends Model {
	public function addAddress($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$this->customer->getId() . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . '' . "', company = '" . '' . "', address_1 = '" . $this->db->escape($data['address_1']) . "', address_2 = '" .'' . "', postcode = '" . '' . "', city = '" . '' . "', zone_id = '" . (int)$data['zone_id'] . "', country_id = '" . (int)$data['country_id'] . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? serialize($data['custom_field']) : '') . "'");
