...
class ControllerProductProduct extends Controller {
	...
	public function index() {
        ...
        $data['tab']=$this->url->link('product/product', 'product_id=' . $product_id);
...
//        print_r($product_info);
 		if ($product_info) {
			...
			$data['short_description'] = $product_info['meta_description'];
			$data['manufacturer'] = $product_info['manufacturer'];
			$data['manufacturers'] = $this->url->link('product/manufacturer/info', 'manufacturer_id=' . $product_info['manufacturer_id']);
			$data['model'] = $product_info['model'];
			$data['material'] = $product_info['material'];
			$data['guarantee'] = $product_info['guarantee'];
			...
			if ($product_info['image']) {
				$data['thumb'] = $this->model_tool_image->resize($product_info['image'], 350, 350);
			} else {
				$data['thumb'] = '';
			}
            ...
            if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
				$data['price_sale'] = $this->currency->format($this->tax->calculate($product_info['price_sale'], $product_info['tax_class_id'], $this->config->get('config_tax')));
			} else {
				$data['price_sale'] = false;
			}

			if ((float)$product_info['special']) {
				$data['special'] = $this->currency->format($this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax')));
				if($data['price']) {$specialper = (($product_info['price'] - $product_info['special'])/$product_info['price']) * 100;
